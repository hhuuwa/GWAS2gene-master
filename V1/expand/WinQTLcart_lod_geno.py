...
def parse_genes_msu(gff_path: str) -> List[Tuple[str, int, int, int, int]]:
    out = []
    pat = re.compile(r"chr(\d+)\tMSU_osa1r7\tgene\t(\d+)\t(\d+)\t")
    id_pat = re.compile(r"ID=(\S+);Name")
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("Usage: python expand/WinQTLcart_lod_geno.py <AREA_TRAIT>")
    ...
    gff = "./basic_data/Rice_MSUv7.gff3"
    if not os.path.exists(gff):
        alt = "./basic_data/Rice_MSUv7/Rice_MSUv7.gff3"
        if os.path.exists(alt):
            gff = alt
    genes = parse_genes_msu(gff)
    out_path = f"{sys.argv[1]}_NAM_LOD_geno.info"
